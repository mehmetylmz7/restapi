...
const wizardModelFields = {
    ...
    invoices: [
        ...
        { key: "olusturma_tarihi", label: "Oluşturma Tarihi (olusturma_tarihi)", required: false, desc: "Fatura oluşturulma tarihi (YYYY-MM-DD HH:MM:SS)" }
    ]
};
...
// ==========================================
// KULLANICI PANELI (USER PORTAL) İŞLEMLERİ
// ==========================================
...
function toggleAuth(event) {
    ...
    if (isRegisterMode) {
        ...
        title.innerHTML = '<i class="fas fa-user-plus" style="color: var(--success);"></i> Kayıt Ol';
        subtitle.textContent = "Yeni bir müşteri hesabı oluşturun";
        toggleText.textContent = "Zaten hesabınız var mı?";
        toggleLink.textContent = "Giriş Yap";
        toggleLink.style.color = "var(--success)";
    } else {
        loginForm.style.display = "block";
        registerForm.style.display = "none";
        title.innerHTML = '<i class="fas fa-lock" style="color: var(--accent-color);"></i> Giriş Yap';
        subtitle.textContent = "Müşteri hesabınıza erişin";
        toggleText.textContent = "Hesabınız yok mu?";
        toggleLink.textContent = "Kayıt Ol";
        toggleLink.style.color = "var(--accent-color)";
    }
...
function initUserPortal() {
    ...
    const authContainer = document.getElementById("user-portal-auth-container");
    const dashboardContainer = document.getElementById("user-portal-dashboard-container");
...
function showPortalTab(tabName) {
    ...
    document.getElementById(`portal-${tabName}-tab`).style.display = "block";

    // Buton stillerini güncelle
    const btnPayments = document.getElementById("btn-portal-payments");
    const btnInvoices = document.getElementById("btn-portal-invoices");
    const btnPay = document.getElementById("btn-portal-pay");

    [btnPayments, btnInvoices, btnPay].forEach(btn => {
        btn.style.background = "transparent";
        btn.style.color = "var(--text-muted)";
        btn.style.boxShadow = "none";
    });

    const activeBtn = document.getElementById(`btn-portal-${tabName}`);
    activeBtn.style.background = "var(--accent-color)";
    activeBtn.style.color = "#fff";

    // Tab'e göre yükleme yap
    if (tabName === 'payments') {
        loadPortalPayments();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ödeme Yap Formu
    const payForm = document.getElementById("portal-pay-form");
    if (payForm) {
        payForm.addEventListener("submit", (e) => {
            e.preventDefault();
            const token = localStorage.getItem("user_access_token");
            const amount = document.getElementById("portal-pay-amount").value;
            const currency = document.getElementById("portal-pay-currency").value;
            const orderId = document.getElementById("portal-pay-order-id").value;
            const msgEl = document.getElementById("portal-pay-msg");
